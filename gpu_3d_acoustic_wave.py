...
source_y = np.int32(grid_size_y / 2)
...
def sim_webgpu():
    p_present = np.zeros((grid_size_z, grid_size_x, grid_size_y), dtype=np.float32)
    p_past = np.zeros((grid_size_z, grid_size_x, grid_size_y), dtype=np.float32)
    p_future = np.zeros((grid_size_z, grid_size_x, grid_size_y), dtype=np.float32)
    lap = np.zeros((grid_size_z, grid_size_x, grid_size_y), dtype=np.float32)

    device = wgpu.utils.get_default_device()
    # for limit in device.limits.items():
    #     print(limit)

    info_int = np.array(
        [
            np.int32(grid_size_z), np.int32(grid_size_x), np.int32(grid_size_y), np.int32(source_z), np.int32(source_x),
            np.int32(source_y), 0
        ],
        dtype=np.int32
    )
    ...
    info_float = np.array(
        [
            np.float32(dz), np.float32(dx), np.float32(dy), np.float32(dt)
        ],
        dtype=np.float32
    )

    cshader = None
    with open('3d_acoustic_wave_shader.wgsl') as shader_file:
        cshader_string = shader_file.read().replace('wsz', f'{wsz}').replace('wsx', f'{wsx}').replace('wsy', f'{wsy}')
    ...
    for i in range(total_time):
        ...
        compute_pass.dispatch_workgroups(grid_size_z // wsz, grid_size_x // wsx, grid_size_y // wsy)
    ...
    return np.asarray(out).reshape((grid_size_z, grid_size_x, grid_size_y)), adapter_info["device"]
